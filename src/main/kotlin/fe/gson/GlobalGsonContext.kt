...
import fe.gson.extension.registerTypeAdapters
import fe.gson.typeadapter.dateTimeAdapters
...
object GlobalGsonContext {
    ...
    fun configure(configure: ConfigureGson = defaultGsonConfiguration): Gson {
        if (_gson == null) {
            return GsonBuilder().setPrettyPrinting().disableHtmlEscaping().apply(configure).create().apply { _gson = this }
        }
